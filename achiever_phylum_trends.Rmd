...
First we can get a broad sense of the data by looking at the various phyla present across all locations and depths together.
...
We can see that two phyla: **Proteobacteria** (orange), **Bacteroidota** (dark blue) are quite dominant, and a number of other phyla are present in lower abundances. Next, we can look at the phylum trends across the various locations: in the three inlets (top panel) compared with the River samples and station QU43 samples
...
You can see that some phyla dominate across all locations (**Proteobacteria** (orange), **Bacteroidota** (dark blue)), while others are less prominent and vary slightly across locations (e.g. **Crenarchaeota** (purple) in marine locations, but absent from rivers, **Actinobacteriota** (navy blue) slightly more abundant in rivers than marine locations)
...
First we can get a broad sense of the data by looking at the various phytoplankton phyla present across all locations and depths together.
...
It is quite clear that **DIATOMS** are a dominant phyla (**Bacillatiophyta**, purple) and **Crytophyceae** (common in freshwater/brackish habitats), **Prymnesiophyceae** (haptophytes) and **Mamiellophyceae** (unicellular green algae) are the next most abundant. Other phyla are present but relatively rare.

Next we can look at the trends based on location:in the three inlets (top panel) compared with the River samples
...
Looking across depths, we see that some phyla are abundant across all depths (diatoms again in purple), whereas some phyla are more or less abundant at different depths (e.g. Cryptophyceae in medium blue). Interesting that they become more abnundant at 200-300m? 
...
Seasonal trends are a bit more abundant with the Phytoplankton data compared to the Bacteria/Archaea data. For example there is some clear diatom seasonality, with highest abundance in Autumn, followed by Summer, Spring and then Winter. Most other phyla also appear to exhibit some degree of seasonality (e.g. **Raphidophyceae** (light green) most abundant in summer, **Mamiellophyceae** (turquoise) most abundant in winter).

In the next R Markdown document ('achiever_ordination_plots.Rmd'), I will start to explore community level trends using ordination methods, as well as examine environmental variables as drivers of these trends.
